...
detail_on() {
  sketchybar --animate tanh 30 --set volume.slider slider.width=$WIDTH
}

detail_off() {
  sketchybar --animate tanh 30 --set volume.slider slider.width=0
}

toggle_detail() {
  INITIAL_WIDTH=$(sketchybar --query volume.slider | jq -r ".slider.width")
